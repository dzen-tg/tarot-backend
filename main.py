...
def get_db_connection():
    ...
    # Очищаем строку подключения от возможных пробелов и символов переноса
    ...
    # Попытка 1: Подключение с принудительным IPv4-разрешением хоста
    try:
        try:
...
def generate_local_tarot_reading(question: str, pre_selected_cards: list) -> dict:
    ...
    Каждая карта толкуется на основе её ранга, масти и контекста вопроса пользователя!
    """
    print("🔮 Активирован сверхдинамический генератор «Вечный Оракул»", flush=True)
    q_lower = question.lower()
    
    # 1. Интеллектуальный разбор темы вопроса искателя
    theme = "общий жизненный путь"
    if any(w in q_lower for w in ["работ", "карьер", "бизнес", "проект", "увольн", "найти", "деньг", "финанс", "youtube", "видео"]):
    ...
    elif any(w in q_lower for w in ["себя", "предназнач", "духов", "выбор", "делать", "почему", "душа"]):
        theme = "самопознание"
    ...
    for idx, card in enumerate(used_cards):
        ...
        # Шаг А. Семантический разбор ранга карты (для Младших Арканов)
        rank_desc = ""
        ranks = ["Туз", "Двойка", "Тройка", "Четверка", "Пятерка", "Шестерка", "Семерка", "Восьмерка", "Девятка", "Десятка", "Паж", "Рыцарь", "Королева", "Король"]
        for r in ranks:
            if r in name:
                if r == "Туз":
                    rank_desc = "воплощает в себе чистый космический импульс, мощный потенциал и новые открывающиеся горизонты."
                elif r == "Двойка":
                    rank_desc = "указывает на необходимость взвешенного выбора, хрупкий баланс сил и поиск внутренней гармонии."
                elif r == "Тройка":
                    rank_desc = "символизирует первые важные успехи, расширение вашего влияния и уверенный рост планов."
                elif r == "Четверка":
                    rank_desc = "призывает к стабильности, потребности в качественном отдыхе, защите границ и разумном консерватизме."
                elif r == "Пятерка":
                    rank_desc = "предупреждает о временных кризисах, борьбе, выходе из зоны комфорта и важных жизненных уроках."
                elif r == "Шестерка":
                    rank_desc = "несет с собой гармонию, заслуженную победу, помощь единомышленников или приятную душевную ностальгию."
                elif r == "Семерка":
                    rank_desc = "призывает к осторожности, стратегическому планированию, терпению или защите от иллюзий."
                elif r == "Восьмерка":
                    rank_desc = "означает кропотливый труд, целенаправленное движение, оттачивание мастерства или выход из тупика."
                elif r == "Девятка":
                    rank_desc = "указывает на вашу личную самодостаточность, скорое достижение желаемого и внутреннюю зрелость."
                elif r == "Десятка":
                    rank_desc = "знаменует триумфальное завершение большого цикла, материальное изобилие и верность традициям."
                elif r == "Паж":
                    rank_desc = "приносит свежие новости, импульс к обучению, новые шансы и яркий юношеский энтузиазм."
                elif r == "Рыцарь":
                    rank_desc = "несет дух стремительного движения, решительных действий, командировок и быстрых перемен."
                elif r == "Королева":
                    rank_desc = "воплощает интуитивную мудрость, глубокую эмоциональную зрелость, мягкое влияние и заботу."
                elif r == "Король":
                    rank_desc = "олицетворяет высшую власть в своей сфере, стабильность, авторитет и мастерство контроля."
                break

        # Шаг Б. Подстройка под масть в зависимости от темы вопроса
        suit_desc = ""
        if "Пентакли" in card_type or "Пентаклей" in name:
            if theme == "финансы и карьера":
                suit_desc = "В вопросах материального (будь то YouTube, видеопроизводство или бизнес) это сулит твердый фундамент, реальную денежную выгоду и окупаемость ваших вложений."
            elif theme == "любовь и отношения":
                suit_desc = "В отношениях этот символ указывает на надежность чувств, готовность строить совместный быт и серьезные долгосрочные намерения."
            else:
                suit_desc = "Это период заземления, заботы о своем здоровье, наведения порядка в повседневных делах и укрепления личных границ."
        elif "Кубки" in card_type or "Кубков" in name:
            if theme == "финансы и карьера":
                suit_desc = "Это призывает вас опереться на творческую интуицию, доверие команды, креативность и искреннее вдохновение вашим делом."
            elif theme == "любовь и отношения":
                suit_desc = "Это период расцвета искренних чувств, глубокого душевного единения, романтики и полного взаимопонимания без масок."
            else:
                suit_desc = "Оно направляет ваше внимание на внутренний мир, исцеление старых душевных травм и поиск эмоционального покоя."
        elif "Мечи" in card_type or "Мечей" in name:
            if theme == "финансы и карьера":
                suit_desc = "Это требует от вас холодного разума, жесткой аналитики, оптимизации расходов и смелости в борьбе с конкурентами."
            elif theme == "любовь и отношения":
                suit_desc = "Это указывает на необходимость честного, хотя порой и непростого разговора для прояснения скрытых моментов."
            else:
                suit_desc = "Оно учит вас ментальной силе, умению говорить твердое 'нет' и решительно освобождаться от разрушающих мыслей."
        elif "Жезлы" in card_type or "Жезлов" in name:
            if theme == "финансы и карьера":
                suit_desc = "Это яркий маркер лидерства, масштабирования вашего бренда, зажигания новых амбициозных идей и победы в проектах."
            elif theme == "любовь и отношения":
                suit_desc = "Это наполняет союз страстью, яркими эмоциями, совместными приключениями и взаимным сексуальным притяжением."
            else:
                suit_desc = "Оно пробуждает ваш внутренний огонь, силу воли, харизму и готовность заявить о себе всему миру."
        else:
            # Трактовка для Старших Арканов на основе темы вопроса
            clean_name = name.replace("Старший Аркан: ", "")
            if theme == "финансы и карьера":
                suit_desc = f"Этот великий Аркан ({clean_name}) приносит глобальные перемены в ваши проекты. Прислушайтесь к космическим подсказкам — перед вами открывается новая судьбоносная дверь."
            elif theme == "любовь и отношения":
                suit_desc = f"Этот Аркан ({clean_name}) указывает на прохождение важного урока души. Ситуация требует от вас мудрости, безусловного принятия и глубокого доверия."
            else:
                suit_desc = f"Аркан ({clean_name}) является знаком вашей духовной трансформации. Происходящие события ведут к обретению истинной внутренней силы."

        # Шаг В. Сборка красивого, уникального толкования карты
        card_text = f"**{pos}** — Аркан **«{name}»**:\n"
        if rank_desc:
            card_text += f"Этот священный символ {rank_desc} {suit_desc}\n"
        else:
        ...
        for model in models:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Выводим в логи точные причины сбоя от Google для разработчика
